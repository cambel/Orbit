...
from .peg_in_hole_cfg import RandomizationCfg, PegInHoleEnvCfg


class PegInHoleEnv(IsaacEnv):
    """Environment for reaching to desired pose for a single-arm manipulator."""

    def __init__(self, cfg: PegInHoleEnvCfg = None, **kwargs):
        # copy configuration
        self.cfg = cfg
    ...
    def _step_impl(self, actions: torch.Tensor):
        ...
        elif self.cfg.control.control_type == "compliance_control":
            # TODO: compute `robot action` in Cartesian space
            robot_action = self.actions
            # set the controller commands
            self._ik_controller.set_command(robot_action)
            # compute the joint commands
            self.robot_actions[:, : self.robot.arm_num_dof] = self._ik_controller.compute(
                self.robot.data.ee_state_w[:, 0:3] - self.envs_positions,
                self.robot.data.ee_state_w[:, 3:7],
    ...
    def _initialize_views(self) -> None:
        ...
        elif self.cfg.control.control_type == "compliance_controller":
            self._ik_controller = DifferentialInverseKinematics(
                self.cfg.control.inverse_kinematics, self.robot.count, self.device
            )
            # note: we exclude gripper from actions in this env
...
class ReachObservationManager(ObservationManager):
    ...
    def arm_dof_pos_normalized(self, env: PegInHoleEnv):
...
class ReachRewardManager(RewardManager):
    ...
    def tracking_robot_position_l2(self, env: PegInHoleEnv):
    ...
    def tracking_robot_position_exp(self, env: PegInHoleEnv, sigma: float):
